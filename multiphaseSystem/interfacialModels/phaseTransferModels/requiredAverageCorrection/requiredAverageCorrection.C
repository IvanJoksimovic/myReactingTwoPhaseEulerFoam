...
Foam::phaseTransferModels::requiredAverageCorrection::dmdt() const
{
    ...
    Info << "-------------------------------------" << endl;
    Info << "totalVolume     = " << totalMeshV      << endl;
    Info << "currentAverage  = " << currentAverage << endl;
    Info << "requiredAverage = " << reguiredAverage << endl;
    Info << "correction      = " << correction      << endl;
    Info << "relaxation      = " << relaxationFactor << endl; 
    Info << "sign            = " << sign << endl;
    /* 
    const volScalarField  dmdt  = relaxationFactor*correction*rho/dt;
    scalar         gdmdt  = abs(gSum(dmdt));
    ...
    {
	    reduction = 1e8/gdmdt;
	    std::cout << "Reduction of dmdt" << std::endl;
    }
    Info << "gdmdt = " << gdmdt << endl;
  */
    Info << "-------------------------------------" << endl;
    
    //return sign*relaxationFactor*cellWeigth*correction*rho/dt; //rho/dt;
    return sign*relaxationFactor*correction*rho/dt;
}
